...
const PengajuanRehab = () => {
  ...
  // Handle map modal for native platforms
  useEffect(() => {
    if (Platform.OS !== 'web' && showMapModal) {
      console.log('📱 Mobile map picker activated');
      setMapLoaded(true);
    }
  }, [showMapModal]);

  const initMap = () => {
    // Only initialize map on web platform with Leaflet library loaded
    if (!window.L || Platform.OS !== 'web') {
      console.warn('⚠️ Leaflet not available or not on web platform');
      return;
    }
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.mapModalContent}>
            ...
            {Platform.OS !== 'web' && (
              <View style={styles.mobileCoordinateInput}>
                <Text style={styles.mobileInputLabel}>Latitude</Text>
                <TextInput
                  style={styles.mobileInput}
                  placeholder="-7.2575"
                  keyboardType="decimal-pad"
                  value={tempCoordinates.lat.toString()}
                  onChangeText={(text) => {
                    const lat = parseFloat(text) || tempCoordinates.lat;
                    setTempCoordinates({ ...tempCoordinates, lat });
                  }}
                />
                
                <Text style={[styles.mobileInputLabel, { marginTop: 16 }]}>Longitude</Text>
                <TextInput
                  style={styles.mobileInput}
                  placeholder="112.7521"
                  keyboardType="decimal-pad"
                  value={tempCoordinates.lng.toString()}
                  onChangeText={(text) => {
                    const lng = parseFloat(text) || tempCoordinates.lng;
                    setTempCoordinates({ ...tempCoordinates, lng });
                  }}
                />
              </View>
            )}
